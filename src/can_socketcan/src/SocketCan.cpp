...
void SocketCan::processCanTxEvent(const can_socketcan::can::ConstPtr& msg)
{
	can_frame tx;
...
void SocketCan::canRxHandler(const boost::system::error_code& error, size_t bytes_transferred)
{
	...
	// XXX
	//ROS_INFO("Received a CAN Message!");
    ...
    if (bytes_transferred)
    {
      ...
      if(flag_pub_active == true) // Only publish if publisher is set up
