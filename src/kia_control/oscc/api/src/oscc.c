...
oscc_result_t oscc_open_fevcan_oscc( unsigned int channel )
{
    oscc_result_t result = OSCC_ERROR;
...
oscc_result_t oscc_open_fevcan_vehicle( unsigned int channel )
{
    oscc_result_t result = OSCC_ERROR;
    ...
    result = init_vehicle_can( can_string_buffer );
...
oscc_result_t oscc_custom_enable(bool brakeEnable, bool throttleEnable, bool steeringEnable)
{
...
oscc_result_t oscc_publish_steering_angle( double angle, double velocity)
{
    oscc_result_t result = OSCC_ERROR;
    ...
    steering_cmd.angle = ( int16_t )(angle * 10);
    steering_cmd.max_velocity = (int16_t)(velocity);

    result = oscc_can_write(
        OSCC_STEERING_ANGLE_CAN_ID,
        (void *) &steering_cmd,
        sizeof(steering_cmd) );
...
oscc_result_t oscc_subscribe_to_steering_angle_reports( void (*callback)(oscc_steering_angle_report_s *report))
{
    ...
    if ( callback != NULL )
    {
        steering_angle_report_callback = callback;
        result = OSCC_OK;
    }
    ...
    if ( global_oscc_can_socket >= 0 )
    {
        ...
        while ( oscc_can_bytes > 0 )
        {
                ...
                && (rx_frame.data[1] == OSCC_MAGIC_BYTE_1) )
            {
                ...
                else if ( rx_frame.can_id == OSCC_STEERING_ANGLE_CAN_REPORT )
                {
                    oscc_steering_angle_report_s *steering_angle_report =
                        ( oscc_steering_angle_report_s* ) rx_frame.data;

                    if ( steering_angle_report_callback != NULL )
                    {
                        steering_angle_report_callback( steering_angle_report );
                    }
                }
